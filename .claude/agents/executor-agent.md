...
> **Codex Backend Dispatch Rule (ADR-18 §S2, amended by ADR-69/73)**: When this delegated agent is selected, use the Codex plugin/native lane and never raw `codex exec`. Delegation itself is proportional; bounded main work may stay direct.
...
Routing SoT: ADR-69 amends ADR-65. Raw `codex exec` is banned. When delegation is selected, use `mir_executor --dispatch`, the user-scope Codex plugin, or native read-only breadth according to the task. A missing preferred lane is not a task blocker when a safe bounded direct path remains.
...
```bash
scripts/mir.sh executor execute --background --dispatch \
...
`--codex-args` is a legacy option name; in `--dispatch` mode its positional prompt is sent to `codex app-server`, not to raw `codex exec`.
...
### Read-only / non-mutating work → plugin/native routing (nothing to merge)

Claude-main investigation/review uses the user-scope `codex:codex-rescue` / `/codex:rescue`
plugin with `--model` / `--effort` from the central routing policy. Codex-main breadth uses only
...
plugin/native routes. If the selected lane is unavailable, report the lane limitation so the parent
...
- **Use a positional prompt in `--codex-args`**, not stdin piping. The dispatch helper extracts the
  positional prompt and sends it to the app-server backend.
...
- **Verify Codex actually ran** by checking the JobRegistry status/result plus app-server dispatch artifacts.
...
- **Auth**: `CODEX_HOME` env must point to the auth.json directory (usually
  `${CODEX_HOME:-$HOME/.codex}`). `codex --version` verifies binary availability only;
  it does not prove authentication or authorize credential access.
