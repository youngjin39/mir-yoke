...
2. If `execution_backend: codex`, do not dispatch it via the Agent tool. Claude→Codex delegation is MCP-only per ADR-69 (`mcp__codex__codex` / `tools/mir_executor/codex_mcp_client.py`). In-repo write/mutating code/TDD/review work routes through `mir_executor … --dispatch` (R4 worktree + deterministic merge gate, MCP backend). Raw `codex exec` is BANNED in all forms; missing MCP path means `BLOCKED`, never exec fallback.
...
`run_dispatch` / `execute --background --dispatch`, backed by `tools/mir_executor/codex_mcp_client.py`;
raw `codex exec` is banned by ADR-69),
