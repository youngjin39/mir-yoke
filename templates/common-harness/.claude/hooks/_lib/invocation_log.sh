...
# Best-effort hook invocation log writer for ignored repository-local evidence.
...
mir_invocation_log_flush() {
  ...
  local log_dir="$project_dir/.mir/runtime"
  mkdir -p "$log_dir" 2>/dev/null || return 0

  MIR_INVOCATION_LOG_PATH="$log_dir/hook-invocations.jsonl" \
